...
async def parse_tables(link_queue: Queue[str | None], data_queue: Queue[list[dict[str, Any]] | None]):
    async def parse(link: str):
        ...
        for row in dataframe.values:
            if (deals_amount := cast_int(row[-1])) and deals_amount > 0 and header_appeared == 2:
                if header_appeared and (row[0] == 'Итого:' or row[0] == 'Итого по секции:'):
                    break
                res.append({
            ...
            if row[0] == 'Единица измерения: Метрическая тонна' or header_appeared and header_appeared < 2:
                header_appeared += 1
